...
export default function PerfilPage() {
  ...
  const canUseCommunityData = isPlus || hasPriceImport;
  ...
  return (
    <div className="space-y-4 pb-16">
      ...
      <Link
        href="/planes"
        className="block bg-white rounded-3xl p-5 border border-gray-200 shadow-2xs hover:shadow-sm transition-shadow"
      >
      ...
      <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-2xs space-y-3">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-700 mb-1.5">
            Límite de distancia para buscar tiendas
          </label>
          <select
            value={family.search_radius_km === null || family.search_radius_km === undefined ? '' : String(family.search_radius_km)}
            onChange={(e) => {
              const value = e.target.value;
              updateFamilySettings({
                search_radius_km: value === '' ? null : Number(value),
              });
            }}
            className="w-full px-3 py-2.5 text-xs bg-gray-50 border border-gray-200 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-emerald-500 focus:bg-white"
          >
          ...
          <p className="text-[10px] text-gray-400 mt-1.5">
            Mandado mostrará solo los comercios dentro de ese radio desde el centro de tus tiendas habituales. Sin límite muestra todos.
          </p>
        </div>
      </div>

      {/* Datos cruzados entre familias */}
      <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-2xs">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${canUseCommunityData ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
              {canUseCommunityData ? <Check className="w-5 h-5" /> : <Lock className="w-5 h-5" />}
            </div>
            <div className="min-w-0">
              ...
              <span className="text-[11px] text-gray-500 leading-snug block mt-0.5">
                {canUseCommunityData
                  ? 'Disponible para tu familia. Podés ver precios de la comunidad de tu pueblo y comparar con tus comercios.'
                  : 'Disponible con Mandado Plus o con el add-on de importación única de precios.'}
              </span>
            </div>
          </div>
          {!canUseCommunityData && (
            <Link
              href="/planes"
              className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 rounded-lg shrink-0"
            >
              Ver planes
            </Link>
          )}
